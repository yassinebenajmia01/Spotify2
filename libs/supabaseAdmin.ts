...
const createOrRetrieveACustomer = async({ email, uuid }: { email: string; uuid: string }) => {
    ...
    return data.stripe_customer_id; // string
};
...
const subscription = subscriptionResponse; // TS now knows this is Stripe.Subscription

    const subscriptionData:Database["public"]["Tables"]["subscriptions"]["Insert"]={
        id:subscription.id,
        user_id:uuid,
        metadata:subscription.metadata,
        // @ts-expect-error Stripe.Subscription type mismatch
        // @ts-expect-error Stripe.Subscription type mismatch
        price_id:subscription.items.data[0].price.id,
        // @ts-expect-error Stripe.Subscription type mismatch
        quantity:subscription.quantity,
        cancel_at_period_end:subscription.cancel_at_period_end,
        cancel_at:subscription.cancel_at ? toDateTime(subscription.cancel_at).toISOString():undefined,
        canceled_at:subscription.canceled_at ? toDateTime(subscription.canceled_at).toISOString():undefined,
        // @ts-expect-error Stripe.Subscription type mismatch
